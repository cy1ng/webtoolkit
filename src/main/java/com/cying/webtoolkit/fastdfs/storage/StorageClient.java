package com.cying.webtoolkit.fastdfs.storage;

import java.io.*;
import java.util.*;
import java.net.*;
import com.cying.webtoolkit.fastdfs.callback.DowloadCallback;
...
import com.cying.webtoolkit.fastdfs.tracker.TrackerClient1;
...
public class StorageClient
{
	...
	*/
	public String[] upload_file(byte[] file_buff,	String file_ext_name, 
	       NameValuePair[] meta_list) throws IOException, Exception
	{
		return this.do_upload_file(null, null, file_buff, file_ext_name, meta_list);
	}
	...
	*/
	       ...
	       NameValuePair[] meta_list) throws IOException, Exception
	{
		...
		try
		{
			...
			{
				meta_buff = ProtoCommon.pack_metadata(meta_list).getBytes(ProtoCommon.DEFAULT_CHARSET);
			}
  ...
  * @param file_offset the start offset of the file
	...
	*/
	public NameValuePair[] get_metadata(String group_name, String remote_filename)throws IOException, Exception
	{
		...
		try
		{
			...
			return ProtoCommon.split_metadata(new String(pkgInfo.body, ProtoCommon.DEFAULT_CHARSET));
		}
		finally
		{
			if (bNewConnection)
	...
	*/
							...
							NameValuePair[] meta_list, byte op_flag) throws IOException, Exception
	{
		...
		try
		{
			...
			filenameBytes = remote_filename.getBytes(ProtoCommon.DEFAULT_CHARSET);
	...
	*/
	protected boolean newWritableStorageConnection(String group_name) throws IOException, Exception
	{
		...
		{
			TrackerClient1 tracker = new TrackerClient1();
