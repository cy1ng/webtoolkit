...
public class ProtoCommon
{
	...
	public static int DEFAULT_TIMEOUT=10000;  //XXX  Unit
...
*/
	public static byte[] packHeader(byte cmd, long pkg_len, byte errno) throws UnsupportedEncodingException
	{
		...
		hex_len = ProtoCommon.long2buff(pkg_len);  //big-endian
...
*/
	public static RecvHeaderInfo recvHeader(InputStream in, byte expect_cmd, long expect_body_len) throws IOException
	{
		...
		header = new byte[TRACKER_PROTO_PKG_LEN_SIZE + 2]; //bodyLen(8)+command(1)+status(1)=10
		...
		pkg_len = ProtoCommon.buff2long(header, 0);  //��ǰ8���ֽ�ת����long
		if (pkg_len < 0)
		{
...
*/
 ...
 {
 	byte[] bsFileId = file_id.getBytes(DEFAULT_CHARSET);
 	byte[] bsKey = secret_key.getBytes(DEFAULT_CHARSET);
